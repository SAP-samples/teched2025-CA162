...
- Identify relevant failure reasons. We'll be using six different "logon failed":
...
> [!NOTE] 
> For failed logon reasons which have more than one entry, select all! In this example, for the first logon failed member (reason 1), select the first two event messages. Confirm with **OK**.
...
- Aggregation 6: Failed RFC/CPIC logons
